...
public class Test11 {
  ...
  public static void main(String[] args) {
    ...
    // 클래스를 사용하기 미리 해당 클래스가 어느 패키지에 있는지 지정한 다음에
    // 사용하는 것이다.
    //      import 패키지명.클래스명;
    // 특정 패키지의 클래스를 많이 사용한다면 다음과 같이 패키지명 다음에 wildcard(*)를 지정 할 수 있다.
    //      import 패키지명.*; 
    // => 이 경우 해당 패키지의 모든 클래스 패키지명 없이 사용할 수 있다.
    // => 서브 패키지는 해당이 안된다.
    // => 단 소스 코드를 읽을 때 어떤 클래스가 어떤 패키지 있는지 바로 확ㅇ딘할 수 없는 불편함이 있다.
