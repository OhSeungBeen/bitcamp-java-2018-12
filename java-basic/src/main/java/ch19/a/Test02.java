...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // => 만약 인스턴스의 값을 초기화 시키기 위해 복잡한 코드를 작성해야 한다면,
    //    인스턴스 블록에 작성하라!
    ...
    // 예1:
    // => Car클래스를 상속 받아 익명 클래스를 정의한다.
    // => 인스턴스를 생성할 때 수퍼 클래스은 Car의 기본생성자를 호출한다.
    //
    Car obj = new Car() {
      @Override
      public void run() {
    ...
    }; // new 명령은 인스턴스를 생성하는 명이기 때문에 끝에 세미콜론(;)을 붙여야 한다.
  ...
  public static void m1() {
    ...
    // 스태틱 멤버는 인스턴스 주소를 담는 this라는 built-int 변수가 없기 때문에
    // 인스턴스 멤버(필드, 메서드, inner 클래스를) 사용할 수 없다.
    
    B obj2; // 레퍼런스 변수를 선언할 때는 nonstatic = inner 클래스를 사용 할 수 있다.
    //obj2 = new B(); // 컴파일 오류!
