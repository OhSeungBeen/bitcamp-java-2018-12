...
public class Test14 {
  public static void main(String[] args) throws Exception{
    ...
    byte[] bytes = {(byte)0xb0, (byte)0xba1, (byte)0xb0, (byte)0xa2, 0x30, 0x31, 0x32, 0x41, 0x42, 0x43};
    // 문자 코드 값이 저장된 바이트 배열로 String 인스턴스 생성
    String s4 = new String(bytes, "euc-kr");
    // 한글이 깨진다. 이유?
    // => String 생성자는 파라미터로 받은 바이트 배열에 ISO-8859-1 문자 코드가 들어 있다고 간주한다.
    //       즉 0xb0 0xa1 값이 한글 '가'가 아니라 0xb0와 0xa1 각각을 영어라 간주하고
    //    ISO-8859-1에 정의된 문자표에 따라 유니코드(UTF-16)으로 바꿔 저장한
    //       0xb0(ISO-8859-1) ==> 0x00b0(UTF-16)
    //       제대로한글을 처리하려면? 생성자에 바이트 배열을 넘겨줄 때 배열에 들어 있는 코드 값이
    ...
    byte[] bytes3 = {(byte)0xea, (byte)0xb0, (byte)0x80, (byte)0xb0, (byte)0x81,
        0x61, 0x62, 0x63,};
