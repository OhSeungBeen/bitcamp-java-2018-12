...
public class Test07 {
  ...
  public static void main(String[] args){
   ...
   //=> UTF-16은 2바이트 크기를 갖는다.
    ...
    // 작은 따옴표 안에 직접 유니코드를 표기할 수 있다.
    System.out.println('\uac00');
    
    //작은 따옴표 안에 직접 유니코드를 지정하는 경우?
    //=> 특수 문자를 출력하고 싶을 때
    //=> 코드에 해당하는 문자를 현재 콘솔 창에 출력할 때는
...
//- 2진수화 시켜서 저장한다.
//
//#문자의 2진수로 변환(인코딩; encoding)하는 규칙 ==> 문자집합(character set)
//1) ASCII (7bit)
//-미국 국가 표준 협회 (ANSI)에서 영어 알파벳, 숫자, 특수 문자 등을 컴퓨터에서 저장할 수 있도록
// 2진수로 정의한 인코딩 규칙
// A(0x41), B(0x42), a(0x61), b(0x62),
//스페이스(0x20) , LF(0a) CR(0d) !(0x21) 
//#(0x23) $(0x24) %(0x25) &(0x26) +(0x2b)
// 0(0x30), ?(0x3f), =(0x3d), @(0x40),

//-참고
// 윈도우 OS에서는 LFCR 두바이트를 사용하여 줄 바꿈을 표시하다.
...
//아이슬란드어, 아일랜드어, 에스파냐어, 영어, 이탈리아어, 페로어, 포르투갈어, 프랑스어(일부[* 2]),
//핀란드어(일부[* 2]), 동유럽의 알바니아어, 아프라카의 스와힐리어, 아프리칸스어.
//유로 기호(€)와 대문자 "Ÿ"는 개정판인 ISO 8859-15에 추가되었다.
//IANA의 문자 집합 ISO-8859-1에 대응하며, HTML 문서의 기본 인코딩이다.
//
// EUC-KR(2byte; 한글 2560자 + 한자 등 포함)
//- 한글 음절 한 개에 대해 인코딩 규치을 정의한 완성형 인코딩 규칙이다.
//- 현재 유통되는 모든 한글 음절을 표현할 수 없다.
//- 가(0xb0a1), 각(0xb0a2), 똘(0xb6ca), 똠(?), 똥(0xb6cb) 등
//
//4) 조합형(2byte; 초성(5bit) + 중성(5bit) + 종성(5bit))
//- 초성: ㄱ(000010), ㄲ(00011), ㄴ(00100), ㄷ(00101) ...
//- 중성: ㅏ(000011), ㅐ(00100), ㅑ(00101), ㅒ(00110) ...
//- 종성: ㄱ(00010), ㄲ(00011), ㄳ(00100), ㄴ(00101) ..
// -예) 꺅(1000 1100 1010 0010 = 0x8ca2)
//
//5) MS949(CP949; 2byte)
//- 윈도우 OS의 인코딩 규칙이다.
//- EUC-KR + 현재 유통되는 한글 음절 추가 = MS949(111172자 + 한국 전용 한자 + 옛한글 등)
//- MS949에 추가된 문자
// 똠(0x8c63), 똡(0x8c64), 등
...
//
//6) Unicode(2btye; UTF-16 =ISO10646)
//-국제 표준 인코딩 규칙이다.
...
//
//8) UTF-8(1~4byte; Unicode Transformation Format)
