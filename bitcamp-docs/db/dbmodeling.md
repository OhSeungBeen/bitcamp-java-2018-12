...
- row(tuple; record) => 데이터 한 개 (여러 컬럼으로 이루어진) 한개. 예) 학생
...
- 식별자? 
  ...
  - 한 개 이상의 컬럼을 구성 된다.
  - 식별자를 key라고 부른다.
  - 수퍼 키 예: 학생(학번, 이름, 전화, 이메일, 학과, 우편번호, 주소, 주민번호)
    - 학번 (O)
    - 주민등록번호 (O)
    - 이메일 (O)
    - 전화번호 (X)
...
### 후보키(candidate key) 선정
  - super key 들 중에서 선정된 최소키를 선정한다.
  - 최소키? 최소한의 컬럼 값 만으로 식별이 가능한 key.
  - 수퍼 키 예: 학생(학번, 이름, 전화, 이메일, 학과, 우편번호, 주소, 주민번호)
    - 학번 (O) : 
    - 주민등록번호 (O)
    - 이메일 (O)
    - (이름,전화) (O) => 이름과 전화 값을 묶은 것 보다 적은 개 수의 후보키가 있다면 가능한 제외하라!
