...
public class Controller05_1 {
  ...
  public String handler1() {
    ...
    // => 출력 콘텐트는 브라우저에서 기본 HTML로 간주한다.
    //    단 한글은 ISO-8859-1 문자표에 정의된 코드가 아니기 때문에 '?' 문자로 변경된다.
  ...
  public String handler3(HttpServletResponse response) {
    ...
    //  HttpSErvletRequest에 대해 다음과 같이 콘텐트 타입을 설정해봐야 소용없다.
  ...
  public HttpEntity<String> handler4(HttpServletResponse response) {
    ...
    // 이 경웨도 출력할 때 ISO-8859-1 문자표의 코드로 변환하여 출력한다.
    // 그래서 한글은 ? 문자로 변환된다.
