...
public class Controller05_1 {
  ...
  // 1) JSP에서 JSON형식의 콘텐트 출력하기
  // 테스트 :
  // http://localhost:8080/java-spring-webmvc/app2/ch05_1/h1
  @GetMapping("h1")
  public void handler1(Model model) {
    model.addAttribute("list",this.list);
  }
  
//2) Google Gson 라이브러리를 사용하여 JSON형식의 콘텐트 출력하기
 // => mvenrepository.com gson 검색하여 라이브러리 정보를 가져온다.
  // => build.gradle eclipse 출력한다.
  // => $gradle eclipse' 실행한다.
  // => 이클립스에서 프로젝트를 리프래시 한다.
 // http://localhost:8080/java-spring-webmvc/app2/ch05_1/h2
...
//3) Google Gson 라이브러리를 사용하여 JSON형식의 콘텐트 출력하기 II
...
// http://localhost:8080/java-spring-webmvc/app2/ch05_1/h3
@GetMapping("h3")
@ResponseBody
